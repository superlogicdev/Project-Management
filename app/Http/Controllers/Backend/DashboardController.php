...
class DashboardController extends Controller
{
    ...
    public function getTotalProjects(Request $request){
		...
		$dataList = array('data' => array(), 'backgroundColor' => array('#cec62e', '#f25961', '#88c136'), 'labels' => array(__('Inprogress'), __('Timeout'), __('Completed')));
		foreach ($data as $row) {
    ...
    public function getStaffStatus(Request $request){
		...
		FROM users 
		WHERE active_id = 1 AND role_id NOT IN(2)
		UNION ALL
		SELECT 0 TotalActiveStaff, COUNT(id) TotalInactiveStaff 
		FROM users 
		WHERE active_id = 2 AND role_id NOT IN(2)) a;";

		$data = DB::select(DB::raw($query));
		$dataList = array('data' => array(), 'backgroundColor' => array('#88c136', '#f25961'), 'labels' => array( __('Active'), __('Inactive')));
		foreach ($data as $row) {
